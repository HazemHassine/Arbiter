...
class ObservationService:
    ...
    async def start(self) -> None:
        ...
        self._stop.clear()
        self._started_at = datetime.now(UTC)
        self._poll_task = asyncio.create_task(self._poll(), name="arbiter-observer")
        if callable(getattr(self.services.docker, "events", None)):
            self._docker_thread = threading.Thread(
                target=self._watch_docker_events, daemon=True, name="arbiter-docker-events"
            )
        ...
        if changed:
            self._publish(
                SystemEvent(
                    ...
                    resource_id="local",
                    action="updated",
                    message="Arbiter observed a topology change",
                )
            )
