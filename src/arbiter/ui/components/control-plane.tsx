  ...
  return (
    <div className={classes("app-shell", collapsed && "sidebar-collapsed")}>
      ...
      <aside className={classes("sidebar", mobileOpen && "mobile-open")}>
        <div className="sidebar-top"><button className="brand" onClick={() => navigate("overview")} aria-label="Arbiter home"><span className="brand-mark"><Command /></span><span><strong>Arbiter</strong><small>local control plane</small></span></button><button className="collapse-button" onClick={() => setCollapsed((value) => !value)} aria-label={collapsed ? "Expand navigation" : "Collapse navigation"}>{collapsed ? <PanelLeftOpen /> : <PanelLeftClose />}</button><button className="mobile-close" onClick={() => setMobileOpen(false)} aria-label="Close navigation"><X /></button></div>
        <button className="workspace-switcher" id="resource-selector" onClick={() => setPaletteOpen(true)} aria-label="Search all local resources"><span><Search /></span><span><small>Inspecting</small><strong>All local resources</strong></span><ChevronLeft /></button>
      ...
      <main className="main">
        <header className="topbar"><button className="mobile-menu" onClick={() => setMobileOpen(true)} aria-label="Open navigation"><Menu /></button><div className="breadcrumbs" aria-label="Current location"><span>Arbiter</span><i>/</i><strong>{viewTitles[view]}</strong></div><div className="top-actions"><ThemeToggle /><span className="sync-state">{lastSync ? `Updated ${lastSync.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Syncing…"}</span><span className={classes("live-pill", !healthy && "offline")} title={healthy ? "Connected to the local daemon" : "Waiting for the local daemon"}><i /> {healthy ? "Live" : "Offline"}</span><button className="command-button" onClick={() => setPaletteOpen(true)} aria-label="Search resources and navigate"><Search /><span>Search resources…</span><kbd>⌘ K</kbd></button><button className="icon-button" onClick={refresh} title="Refresh current view" aria-label="Refresh current view"><RefreshCw /></button><button className="button primary ask-button" onClick={() => navigate("agent")}><Sparkles /> Ask Arbiter</button></div></header>
