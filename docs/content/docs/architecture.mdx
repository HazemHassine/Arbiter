...
Business logic does not live in the UI, CLI, or API routes. Those layers translate requests into calls to shared services. This prevents one interface from bypassing the safety rules or implementing behavior differently from another interface.
...
- `ports`: parses Linux `ss`, resolves processes through `/proc`, correlates Docker/Compose metadata, finds predictable free ports, and detects duplicate project claims.
...
These browser controls protect the local service from hostile websites; they are
not user authentication. Non-loopback exposure still requires an external TLS
and authentication boundary, an exact `ARBITER_TRUSTED_HOSTS` entry, and a network
policy appropriate to the workstation.

See the [security audit](/docs/security) for findings, verification evidence, and
