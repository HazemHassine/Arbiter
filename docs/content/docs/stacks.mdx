...
description: Multi-project environment profiles, 1-click context switching, topological DAG boot ordering (Kahn's algorithm), and live dependency readiness gates.
---
...
| `POST` | `/api/v1/stacks/{id}/readiness/authorizations` | Request scoped approvals for non-local gates |
| `GET` | `/api/v1/readiness/authorizations` | List persisted readiness grants |
